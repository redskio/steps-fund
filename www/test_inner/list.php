...
//$query = "SELECT * FROM invest_page ORDER BY invest_num DESC LIMIT $no, $page_size";
//$result = mysql_query($query, $conn);
$db->query = "SELECT * FROM invest_page ORDER BY invest_num DESC LIMIT $no, $page_size";
$db->DBQ();
$result = $db->result->fetch_assoc;
// 총 게시물 수 를 구한다.
$db->query = "SELECT count(*) FROM invest_page";
$db->DBQ();
$result = $db->result->fetch_row();
$total_row = $result[0];
//$result_count=mysql_query("SELECT count(*) FROM invest_page",$conn);
...
<!-- 리스트 부분 시작 -->
<?
for($i=1;$row = $db->result->fetch_assoc();$i++)
{
...
<tr>
    ...
    <td height=20 bgcolor=white align=center>
        <a href="read.php?id=<?=$row[invest_num]?>&no=<?=$no?>">
        <?=$row[invest_rate]?></a>
    </td>
    <!-- 번호 끝 -->
    <!-- 제목 -->
    <td height=20 bgcolor=white>&nbsp;
        <a href="read.php?id=<?=$row[invest_rate]?>&no=<?=$no?>">
        <?=strip_tags($row[invest_rate], '<b><i>');?></a>
    </td>
    <!-- 제목 끝 -->
    <!-- 이름 -->
    <td align=center height=20 bgcolor=white>
        <font color=black>
        <a href="mailto:<?=$row[invest_rate]?>"><?=$row[invest_rate]?></a>
        </font>
    </td>
    <!-- 이름 끝 -->
    <!-- 날짜 -->
    <td align=center height=20 bgcolor=white>
        <font color=black><?=$row[invest_rate]?></font>
    </td>
    <!-- 날짜 끝 -->
    <!-- 조회수 -->
    <td align=center height=20 bgcolor=white>
